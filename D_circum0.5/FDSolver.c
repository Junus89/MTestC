...
void fifthLoop(double OmegaR,double Omega, double MaX, double MaY, double MaZ, double ka, double **pF)
{
  ...
  for(int i=0;i<TNum;i++)
    ...
    Vz[i] = 0.0*One[i];
		...
		Q[i] = 0.0*cos(OmegaM*Time[i])*A;
		
		//Lx[i] = cos(OmegaM*Time[i])*(cos(OmegaR*Time[i]+atan2(DY,DX)-cos(OmegaR*Time[i]-atan2(DY,DX))))*A;
		//Ly[i] = cos(OmegaM*Time[i])*(sin(OmegaR*Time[i]+atan2(DY,DX)+sin(OmegaR*Time[i]-atan2(DY,DX))))*A;
		Lx[i] = -cos(OmegaM*Time[i])*sin(OmegaR*Time[i]+atan2(DY,DX)+Theta)*A;//-sin(OmegaR*Time[i]-atan2(DY,DX))))*A;
		Ly[i] = cos(OmegaM*Time[i])*cos(OmegaR*Time[i]+atan2(DY,DX)+Theta)*A;//+cos(OmegaR*Time[i]-atan2(DY,DX))))*A;
		Lz[i] =0.0*One[i];
